                    ...
                    if not messages:
                        ...
                        for model_name in MODELS:
                            for attempt in range(3):
                                try:
                                    st.write(f"🤖 Trying `{model_name}` (attempt {attempt+1})...")
                        ...
                        if ai_resp:
                            ...
                            if "503" in err_str:
                                st.warning("⏱️ All AI models are currently overloaded. Please wait 1–2 minutes and try again.")
                            elif "429" in err_str:
                                st.warning("⏱️ Rate limit hit. Wait 60s and try again.")
                            else:
